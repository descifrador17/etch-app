#!/usr/bin/env ruby
# Generates Recall.xcodeproj (app target + linked local RecallKit package).
# Re-runnable: regenerates the project from scratch each time.
require 'xcodeproj'
require 'fileutils'

repo = File.expand_path('..', __dir__)
proj_dir = File.join(repo, 'Recall')
proj_path = File.join(proj_dir, 'Recall.xcodeproj')

FileUtils.rm_rf(proj_path)
project = Xcodeproj::Project.new(proj_path)

DEPLOYMENT = '26.0'
BUNDLE_ID  = 'com.recall.Recall'

# ---- App target ----------------------------------------------------------
app = project.new_target(:application, 'Recall', :ios, DEPLOYMENT)

# Source + resource files live under Recall/Recall
app_root = File.join(proj_dir, 'Recall')
group = project.main_group.new_group('Recall', 'Recall')
...
# ---- UI test target ------------------------------------------------------
uitests = project.new_target(:ui_test_bundle, 'RecallUITests', :ios, DEPLOYMENT)
uitest_root = File.join(proj_dir, 'RecallUITests')
uitest_group = project.main_group.new_group('RecallUITests', 'RecallUITests')
...
uitests.build_configurations.each do |config|
  ...
  s['SWIFT_VERSION']               = '6.0'
  s['PRODUCT_BUNDLE_IDENTIFIER']   = 'com.recall.RecallUITests'
  s['GENERATE_INFOPLIST_FILE']     = 'YES'
  s['TEST_TARGET_NAME']            = 'Recall'
...
app.build_configurations.each do |config|
  ...
  s['CODE_SIGN_ENTITLEMENTS']                 = 'Recall/Recall.entitlements'
...
scheme.save_as(proj_path, 'Recall', true)
